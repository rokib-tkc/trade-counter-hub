...
function renderCalendar(){const days=[0,1,2,3,4].map(i=>addDays(state.weekStart,i));let html='<div class="calendar"><div class="calendar-grid"><div class="cal-cell cal-head"></div>'+days.map(d=>`<div class="cal-cell cal-head">${d.toLocaleDateString("en-GB",{weekday:"short",day:"numeric",month:"short"})}</div>`).join("");for(const time of timeSlots()){html+=`<div class="cal-cell time-cell">${time}</div>`;for(const d of days){const date=localISODate(d);const cards=filtered().filter(b=>b.collectionDate===date&&normaliseTime(b.collectionTime)===time).map(card).join("");html+=`<div class="cal-cell slot" data-date="${date}" data-time="${time}">${cards}</div>`}}html+='</div></div>';$("calendarView").innerHTML=html;wireCards()}
...
function openBooking(id){state.editing=id?state.bookings.find(x=>x.id===id):null;const b=state.editing||{collectionDate:localISODate(new Date()),collectionTime:"09:00",status:"Waiting",priority:"Normal",estimatedMinutes:30};b.collectionTime=normaliseTime(b.collectionTime)||"09:00";$("modalTitle").textContent=id?`Edit ${b.orderNumber}`:"New booking";for(const key of ["bookingId","orderNumber","customer","collectionDate","collectionTime","orderSize","estimatedMinutes","paymentType","assignedPicker","status","priority","vehicleType","bookedBy","notes"])$(key).value=b[key==="bookingId"?"id":key]??"";$("deleteBtn").classList.toggle("hidden",!id);$("modal").classList.remove("hidden")}
function closeBooking(){$("modal").classList.add("hidden")}
async function saveForm(e){e.preventDefault();const fields=["bookingId","orderNumber","customer","collectionDate","collectionTime","orderSize","estimatedMinutes","paymentType","assignedPicker","status","priority","vehicleType","bookedBy","notes"];const b={};fields.forEach(k=>b[k==="bookingId"?"id":k]=$(k).value);b.collectionTime=normaliseTime(b.collectionTime);try{await api("save",{payload:JSON.stringify(b)});closeBooking();showToast("Booking saved");loadData()}catch(err){showToast(err.message)}}
