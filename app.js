...
/* ===== v1.3.7 professional operations controller ===== */
state.activityCache=state.activityCache||{};
state.specialBookings=[];
state.metaLastLoaded=Number(state.metaLastLoaded||0);
state.presence=state.presence||[];

function bookingTypeLabel(b){return b.bookingType==='Advance Large Order'?'Large':b.bookingType==='Walk in'?'Walk-in':'Small'}
function activeOperationalBookings(){return filtered().filter(b=>b.pickStatus!=='Cancelled'&&b.collectionStatus!=='Cancelled')}
function isOverdueCollection(b){return b.collectionDate<localDate(operationalNow())&&b.collectionStatus!=='Collected'&&b.pickStatus!=='Cancelled'}

function renderDashboard(){
  const today=localDate(operationalNow()),todays=state.bookings.filter(b=>b.collectionDate===today&&b.pickStatus!=='Cancelled');
  const advance=todays.filter(b=>b.bookingType==='Advance Large Order').sort(sortAdvanceBoard);
  const smaller=todays.filter(b=>b.bookingType!=='Advance Large Order').sort(sortSmallBoard);
  const waiting=todays.filter(b=>b.collectionStatus==='Customer Waiting'&&b.collectionStatus!=='Collected').sort((a,b)=>String(a.collectionTime||'99:99').localeCompare(String(b.collectionTime||'99:99')));
  const active=state.team.filter(t=>t.active&&t.role!=='Manager');
  const queueRows=rows=>rows.map(b=>`<button class="queue-row" data-id="${safe(b.id)}"><strong>${safe(b.collectionTime||'Anytime')}</strong><span>${safe(b.orderNumber)} · ${safe(b.customer)}</span><span class="queue-type ${b.bookingType==='Advance Large Order'?'large':b.bookingType==='Walk in'?'walk-in':'small'}">${bookingTypeLabel(b)}</span><span class="pill status ${statusSlug(displayStatus(b))}">${safe(displayStatus(b))}</span></button>`).join('');
  const workload=active.map(t=>({user:t,orders:todays.filter(b=>b.assignedPicker===t.name&&b.collectionStatus!=='Collected'&&b.pickStatus!=='Cancelled').sort(sortBookings)}));
  $('pageBody').innerHTML=`<div class="dashboard-grid">
    <section class="dashboard-section"><h3>Today’s Queue</h3><h4>Advance Picks</h4>${advance.length?queueRows(advance):showEmpty('No advance picks today')}<h4>Small Orders / Walk-ins</h4>${smaller.length?queueRows(smaller):showEmpty('No small orders or walk-ins today')}</section>
    <section class="dashboard-section"><h3>Customer Waiting</h3>${waiting.length?queueRows(waiting):showEmpty('No customers currently waiting')}</section>
    <section class="dashboard-section" style="grid-column:1/-1;border-top:1px solid var(--line);border-right:0"><h3>Team Workload</h3>${workload.length?workload.map(w=>`<div class="workload-person"><div class="workload-person-head"><strong>${safe(w.user.name)}</strong><span>${w.orders.length} active order${w.orders.length===1?'':'s'}</span></div>${w.orders.length?w.orders.map(b=>`<button class="workload-order" data-id="${safe(b.id)}"><strong>${safe(b.collectionTime||'Anytime')}</strong><span>${safe(b.orderNumber)} · ${safe(b.customer)}<small>${bookingTypeLabel(b)} · ${safe(displayStatus(b))}</small></span><span class="queue-type ${b.bookingType==='Advance Large Order'?'large':b.bookingType==='Walk in'?'walk-in':'small'}">${bookingTypeLabel(b)}</span></button>`).join(''):showEmpty('No active orders assigned')}</div>`).join(''):showEmpty('No active team members configured')}</section>
  </div>`;
  document.querySelectorAll('[data-id].queue-row,[data-id].workload-order').forEach(x=>x.onclick=()=>openOrder(x.dataset.id));
}
...
function notifications(){
  ...
  state.bookings.forEach(b=>{
    let n=null;
    if(b.collectionStatus==='Customer Waiting'&&b.pickStatus!=='Ready'&&b.collectionStatus!=='Collected')n={type:'Customer waiting',severity:1,b,msg:`Immediate action: customer waiting while order is ${b.pickStatus||'Waiting'}`};
    else if(b.pickStatus==='Problem')n={type:'Problem',severity:2,b,msg:'Order has a problem requiring action'};
    else if(isLate(b))n={type:'Overdue',severity:3,b,msg:'Collection time has passed'};
    else if(needsPickSoon(b))n={type:'Pick due',severity:4,b,msg:'Not ready within 30 minutes of collection'};
    else if(b.pickStatus==='Cancelled')n={type:'Cancelled',severity:5,b,msg:'Booking is cancelled'};
    if(n){const prev=byOrder.get(b.orderNumber);if(!prev||n.severity<prev.severity)byOrder.set(b.orderNumber,n)}
  });
  return [...byOrder.values()].sort((a,b)=>a.severity-b.severity||String(a.b.collectionTime||'99:99').localeCompare(String(b.b.collectionTime||'99:99')));
}

function renderTimedCalendar(){
  const days=visibleDays(),bookings=activeOperationalBookings().filter(b=>b.bookingType==='Advance Large Order'),current=currentTimeRowInfo(),todayVisible=days.some(d=>localDate(d)===current?.today);let html=`<div class="calendar-wrap ${state.viewMode==='day'?'day-view':''}"><div class="calendar-grid"><div class="cal-cell cal-head"></div>${days.map(d=>`<div class="cal-cell cal-head">${d.toLocaleDateString('en-GB',{weekday:'short',day:'numeric',month:'short'})}</div>`).join('')}`;for(const time of timeSlots()){const currentRow=todayVisible&&current&&current.time===time,style=currentRow?` style="--current-offset:${current.offset}%"`:'';html+=`<div class="cal-cell time-cell ${currentRow?'current-time-cell':''}"${style}>${time}</div>`;for(const d of days){const date=localDate(d),block=isBlocked(date,time),slot=bookings.filter(b=>b.collectionDate===date&&normaliseTime(b.collectionTime)===time).sort(sortAdvanceBoard);html+=`<div class="cal-cell slot ${block?'blocked':''} ${currentRow?'current-time-cell':''}"${style} data-date="${date}" data-time="${time}">${block?safe(block.label||'Blocked'):(slot.length?slot.map(b=>card(b)).join(''):'')}</div>`}}html+='</div></div>';$('pageBody').innerHTML=html;wireCardsV137();
}
function renderSmallCalendar(){const days=visibleDays(),orders=activeOperationalBookings().filter(x=>['Small Order','Walk in'].includes(x.bookingType));$('pageBody').innerHTML=`<div class="small-calendar">${days.map(d=>{const date=localDate(d),col=orders.filter(x=>x.collectionDate===date).sort(sortSmallBoard);return `<section class="day-column" data-date="${date}"><h3>${d.toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'short'})}</h3>${col.length?col.map(x=>card(x,{showTime:true})).join(''):showEmpty('No small orders','Drag a booking here or add a new booking.')}</section>`}).join('')}</div>`;wireCardsV137()}
function renderBoard(){const all=activeOperationalBookings().filter(b=>state.viewMode==='week'||b.collectionDate===localDate(state.day));const renderSection=(title,items,sorter)=>`<div class="board-section"><h2>${title}</h2><div class="board">${PICK_STATUSES.filter(s=>s!=='Cancelled').map(s=>{const col=items.filter(b=>b.pickStatus===s).sort(sorter);return `<section class="board-col"><div class="board-head"><span>${s}</span><span>${col.length}</span></div><div class="board-list" data-status="${s}">${col.length?col.map(b=>card(b,{showTime:true})).join(''):showEmpty('No orders in this stage')}</div></section>`}).join('')}</div></div>`;$('pageBody').innerHTML=renderSection('Advance Picks',all.filter(b=>b.bookingType==='Advance Large Order'),sortAdvanceBoard)+renderSection('Small Orders / Walk ins',all.filter(b=>b.bookingType!=='Advance Large Order'),sortSmallBoard);wireCardsV137()}

function wireCardsV137(){
  document.querySelectorAll('.booking-card').forEach(el=>{el.onclick=()=>openOrder(el.dataset.id);el.ondragstart=e=>{e.dataTransfer.setData('text/plain',el.dataset.id);el.classList.add('is-moving')};el.ondragend=()=>el.classList.remove('is-moving')});
  const bind=(selector,makeChanges)=>document.querySelectorAll(selector).forEach(el=>{el.ondragover=e=>{e.preventDefault();el.classList.add('drag-over')};el.ondragleave=()=>el.classList.remove('drag-over');el.ondrop=async e=>{e.preventDefault();el.classList.remove('drag-over');el.classList.add('drop-saving');try{await quickUpdate(e.dataTransfer.getData('text/plain'),makeChanges(el),{source:'Drag and drop'})}finally{el.classList.remove('drop-saving')}}});
  bind('.slot:not(.blocked)',el=>({collectionDate:el.dataset.date,collectionTime:el.dataset.time}));bind('.day-column',el=>({collectionDate:el.dataset.date}));bind('.board-list',el=>({pickStatus:el.dataset.status}));
}

async function loadActivityForSelected(){if(!state.selected)return[];const id=state.selected.id;if(state.activityCache[id])return state.activityCache[id];const r=await api('activity',{id});state.activityCache[id]=r.activity||[];return state.activityCache[id]}
function taskSelect(label,id,value,options,disabled=false,note=''){return `<div class="task-row"><div><strong>${safe(label)}</strong>${note?`<div class="task-note ${note.includes('Paid')?'task-warning':''}">${safe(note)}</div>`:''}</div><select data-task="${id}" ${disabled?'disabled':''}>${options.map(o=>`<option ${o===value?'selected':''}>${safe(o)}</option>`).join('')}</select></div>`}
function taskCheck(label,id,checked){return `<div class="task-row"><label><input type="checkbox" data-task="${id}" ${checked?'checked':''}>${safe(label)}</label></div>`}
async function renderOrderTab(tab){
  state.activeOrderTab=tab;document.querySelectorAll('[data-order-tab]').forEach(x=>x.classList.toggle('active',x.dataset.orderTab===tab));const b=state.selected;if(!b)return;
  $('orderPanelTitle').textContent=b.orderNumber||'Order';$('orderPanelStatus').innerHTML=`<span class="pill status ${statusSlug(displayStatus(b))}">${safe(displayStatus(b))}</span>`;
  if(tab==='activity'){$('orderPanelContent').innerHTML=showEmpty('Loading activity…');try{const a=(await loadActivityForSelected()).sort((x,y)=>String(y.timestamp).localeCompare(String(x.timestamp)));$('orderPanelContent').innerHTML=`<div class="order-summary">${a.length?a.map(x=>`<div class="notice"><strong>${safe(x.action)}</strong><span>${safe(x.details||'')}</span><small>${safe(x.performedBy)} · ${safe(formatDateTime(x.timestamp))}${x.source?' · '+safe(x.source):''}</small></div>`).join(''):showEmpty('No activity recorded')}</div>`}catch(e){$('orderPanelContent').innerHTML=showEmpty('Activity unavailable',e.message)}return}
  if(tab==='notes'){$('orderPanelContent').innerHTML=`<div class="order-summary"><div class="summary-hero"><strong>Order notes</strong><p>${safe(b.notes||'No notes have been added.')}</p></div>${b.paymentComments?`<div class="summary-row"><span>Payment comments</span><strong>${safe(b.paymentComments)}</strong></div>`:''}</div>`;return}
  if(tab==='picking'){
    const paid=b.paymentStatus==='Paid';let html=`<div class="picking-panel"><div class="picking-summary"><span class="pill status ${statusSlug(b.pickStatus)}">${safe(b.pickStatus)}</span><span class="pill ${paid?'paid':'held'}">${safe(b.paymentStatus||'Payment Required')}</span>${b.paintedOrder?'<span class="pill requirement">Painted Order</span>':''}${b.madeToMeasure?'<span class="pill requirement">Made to Measure</span>':''}</div><div class="task-group"><h4>Picking progress</h4>${taskSelect('Pick Status','pickStatus',b.pickStatus,['Waiting','Picking','Ready','Problem'])}${taskCheck('Pick Note Printed & Checked','pickNoteChecked',b.pickNoteChecked)}${b.drillingRequired?taskSelect('Drilling','drillingStatus',b.drillingStatus||'Not Drilled Yet',['Not Drilled Yet','In Drilling','Drilling Complete'],!paid,'Payment must be Paid before selecting In Drilling'):''}${b.parkwayItems?taskSelect('Parkway Items','parkwayStatus',b.parkwayStatus||'Parkway Items Requested',['Parkway Items Requested','Parkway Items Received']):''}${b.airPicks?taskCheck('Air Picks Actioned','airPicksActioned',b.airPicksActioned):''}${b.osItems?taskSelect('O/S Items','osStatus',b.osStatus||'O/S Requested',['O/S Requested','O/S Actioned']):''}${taskCheck('Pick Photos Complete','pickPhotosComplete',b.pickPhotosComplete)}${taskCheck('Order Checked','orderChecked',b.orderChecked)}</div></div>`;$('orderPanelContent').innerHTML=html;document.querySelectorAll('[data-task]').forEach(el=>el.onchange=()=>{const val=el.type==='checkbox'?el.checked:el.value;quickUpdate(b.id,{[el.dataset.task]:val},{source:'Picking tab'})});return;
  }
  const req=requirementNames(b),valEx=money(b.orderValueExVAT||0),valInc=money(b.orderValueIncVAT||0);$('orderPanelContent').innerHTML=`<div class="order-summary"><div class="summary-hero"><h3>${safe(b.customer)}</h3><p>${safe(b.bookingType)} · ${Number(b.itemCount||0)} items</p></div><div class="tracker">${['Waiting','Picking','Ready','Collected'].map((s,i)=>`<div class="tracker-step ${trackerDone(b,i)?'done':''}">${s}</div>`).join('')}</div><div class="summary-row"><span>Collection</span><strong>${safe(b.collectionDate)}${b.collectionTime?' at '+safe(b.collectionTime):''}</strong></div><div class="summary-row"><span>Assigned picker</span><strong>${safe(b.assignedPicker||'Unassigned')}</strong></div><div class="summary-row"><span>Payment</span><strong>${safe(b.paymentType||'—')} · ${safe(b.paymentStatus||'—')}</strong></div>${b.paymentComments?`<div class="summary-row"><span>Payment comments</span><strong>${safe(b.paymentComments)}</strong></div>`:''}<div class="summary-row"><span>Account</span><strong>${safe(b.accountStatus||'—')}</strong></div><div class="summary-row"><span>Order value</span><strong>${valEx} ex VAT<br><small>${valInc} inc VAT</small></strong></div><h4>Order requirements</h4><div class="requirement-pills">${req.length?req.map(r=>`<span class="pill requirement">${safe(r)}</span>`).join(''):'<span class="pill">No special requirements</span>'}</div></div>`;
  updatePanelButtonsV137(b);
}
function updatePanelButtonsV137(b){$('panelWaiting').disabled=b.collectionStatus==='Customer Waiting'||b.collectionStatus==='Collected'||b.pickStatus==='Cancelled';$('panelReady').disabled=b.pickStatus==='Ready'||b.collectionStatus==='Collected';$('panelCollected').disabled=b.collectionStatus==='Collected'||b.pickStatus!=='Ready';$('panelReleased').disabled=b.accountStatus!=='Held';$('panelPaid').disabled=b.paymentStatus==='Paid'}

function paymentPrompt(){return new Promise(resolve=>{const el=document.createElement('div');el.className='payment-prompt';el.innerHTML=`<div class="payment-prompt-card"><h3>Select Payment Type</h3><p>A Payment Type is required before marking this order Paid.</p><select>${PAYMENT_TYPES.map(x=>`<option>${safe(x)}</option>`).join('')}</select><div class="payment-prompt-actions"><button data-cancel>Cancel</button><button class="primary" data-ok>Mark Paid</button></div></div>`;document.body.appendChild(el);el.querySelector('[data-cancel]').onclick=()=>{el.remove();resolve(null)};el.querySelector('[data-ok]').onclick=()=>{const v=el.querySelector('select').value;el.remove();resolve(v)}})}

async function quickUpdate(id,changes,opts={}){const idx=state.bookings.findIndex(b=>b.id===id);if(idx<0)return;const before={...state.bookings[idx]},proposed={...before,...changes};if(proposed.collectionStatus==='Collected'&&proposed.pickStatus!=='Ready'){showToast('Mark the order Ready before marking it Collected.','error');return}if(proposed.pickStatus==='Ready'&&proposed.accountStatus!=='Released'&&state.user.role!=='Manager'){showToast('Release the account before marking the order Ready.','error');return}if(proposed.paymentStatus==='Paid'&&!proposed.paymentType){showToast('Select a Payment Type before marking the order Paid.','error');return}state.bookings[idx]=proposed;if(state.selected&&state.selected.id===id)state.selected=proposed;renderAll();if(state.selected&&!$('orderPanel').classList.contains('hidden'))renderOrderTab(state.activeOrderTab||'details');try{const payload={...changes,version:Number(before.version||0),source:opts.source||'Quick action'};const r=await api('quick',{id,changes:JSON.stringify(payload)});state.bookings[idx]=r.booking;if(state.selected&&state.selected.id===id)state.selected=r.booking;state.dataRevision=r.revision??state.dataRevision;delete state.activityCache[id];showToast('Order updated');renderAll();if(state.selected&&!$('orderPanel').classList.contains('hidden'))renderOrderTab(state.activeOrderTab||'details')}catch(e){state.bookings[idx]=before;if(state.selected&&state.selected.id===id)state.selected=before;renderAll();showToast(`Update failed: ${e.message}`,'error');throw e}}

function openBooking(id){state.editing=id?state.bookings.find(x=>x.id===id):null;const b=state.editing||{collectionDate:localDate(operationalNow()),bookingType:'Advance Large Order',pickStatus:'Waiting',paymentStatus:'Payment Required',accountStatus:'Released',priority:'Normal',version:0};$('modalTitle').textContent=id?`Edit ${b.orderNumber}`:'New Booking';const fields=['bookingId','orderNumber','customer','bookingType','itemCount','collectionDate','collectionTime','assignedPicker','priority','paymentType','paymentComments','paymentStatus','accountStatus','pickStatus','collectionStatus','bookedBy','orderValueExVAT','orderValueIncVAT','notes'];fields.forEach(k=>{const el=$(k),key=k==='bookingId'?'id':k;if(el)el.value=b[key]??''});['drillingRequired','parkwayItems','airPicks','osItems','paintedOrder','madeToMeasure'].forEach(k=>$(k).checked=!!b[k]);updateTimeOptions();$('collectionTime').value=b.collectionTime||'';$('deleteBookingBtn').classList.toggle('hidden',!id||state.user?.role!=='Manager');$('cancelBookingBtn').classList.toggle('hidden',!id||b.pickStatus==='Cancelled');$('bookingModal').classList.remove('hidden');resetBookingModalScroll();calcVat()}
async function saveBooking(e){e.preventDefault();if(!validWeekday($('collectionDate').value)){showToast('Collection dates must be Monday to Friday.','error');return}if($('paymentStatus').value==='Paid'&&!$('paymentType').value){showToast('Select a Payment Type before marking the order Paid.','error');$('paymentType').focus();return}if($('collectionStatus').value==='Collected'&&$('pickStatus').value!=='Ready'){showToast('An order can only be marked Collected after it is Ready.','error');return}const fields=['bookingId','orderNumber','customer','bookingType','itemCount','collectionDate','collectionTime','assignedPicker','priority','paymentType','paymentComments','paymentStatus','accountStatus','pickStatus','collectionStatus','bookedBy','orderValueExVAT','notes'],b={};fields.forEach(k=>b[k==='bookingId'?'id':k]=$(k).value);['drillingRequired','parkwayItems','airPicks','osItems','paintedOrder','madeToMeasure'].forEach(k=>b[k]=$(k).checked);const oldIndex=state.bookings.findIndex(x=>x.id===b.id),old=oldIndex>=0?{...state.bookings[oldIndex]}:null;b.version=Number(old?.version||0);b.source='Booking form';const temp={...(old||{}),...b,id:b.id||`temp-${Date.now()}`,orderValueIncVAT:Number(b.orderValueExVAT||0)*(1+Number(state.config.VAT_PERCENT||20)/100)};if(oldIndex>=0)state.bookings[oldIndex]=temp;else state.bookings.push(temp);$('bookingModal').classList.add('hidden');renderAll();try{const r=await api('save',{payload:JSON.stringify(b)});const idx=state.bookings.findIndex(x=>x.id===temp.id);if(idx>=0)state.bookings[idx]=r.booking;delete state.activityCache[r.booking.id];state.dataRevision=r.revision??state.dataRevision;showToast('Booking saved successfully');renderAll()}catch(err){if(oldIndex>=0)state.bookings[oldIndex]=old;else state.bookings=state.bookings.filter(x=>x.id!==temp.id);renderAll();showToast(`Booking was not saved: ${err.message}`,'error')}}
async function cancelCurrentBooking(){if(!state.editing)return;if(!confirm(`Cancel ${state.editing.orderNumber}? The booking will move to Cancelled Orders.`))return;try{await quickUpdate(state.editing.id,{pickStatus:'Cancelled',collectionStatus:'Cancelled'},{source:'Booking form'});$('bookingModal').classList.add('hidden');showToast('Order cancelled')}catch(_){}}

async function renderOperationalList(type){setLoading(true,type==='cancelled'?'Loading cancelled orders…':'Loading overdue collections…');try{const r=await api('operationalList',{type});state.specialBookings=r.bookings||[];$('pageBody').innerHTML=`<div class="menu-list-page">${state.specialBookings.length?state.specialBookings.map(b=>`<button class="record-row" data-special-id="${safe(b.id)}"><span class="record-date">${safe(b.collectionDate)}</span><strong>${safe(b.orderNumber)}</strong><span>${safe(b.customer)}<small>${bookingTypeLabel(b)}${b.collectionTime?' · '+safe(b.collectionTime):''}</small></span><span class="pill status ${statusSlug(b.pickStatus)}">${safe(b.pickStatus)}</span><span>${type==='cancelled'?safe(formatDateTime(b.cancelledAt||b.updatedAt)):safe(b.accountStatus||'')}</span></button>`).join(''):showEmpty(type==='cancelled'?'No cancelled orders':'No overdue collections',type==='cancelled'?'Cancelled bookings will appear here for review.':'All historic collections have been resolved.')}</div>`;document.querySelectorAll('[data-special-id]').forEach(x=>x.onclick=()=>{const b=state.specialBookings.find(y=>y.id===x.dataset.specialId);if(!b)return;state.selected=b;$('orderPanel').classList.remove('hidden');renderOrderTab('details')})}catch(e){showToast(e.message,'error')}finally{setLoading(false)}}

function applySyncResponse(r,seq){if(seq&&seq<state.appliedSyncSequence)return;state.appliedSyncSequence=seq||state.appliedSyncSequence;if(r.serverTime){const server=new Date(r.serverTime);if(!isNaN(server))state.serverOffsetMs=server.getTime()-Date.now()}state.dataRevision=r.revision??state.dataRevision;state.lastSuccessfulSync=Date.now();if(r.changed&&Array.isArray(r.bookings)){const selectedId=state.selected?.id;state.bookings=r.bookings;if(selectedId)state.selected=state.bookings.find(b=>b.id===selectedId)||state.selected;renderAll();if(state.selected&&!$('orderPanel').classList.contains('hidden'))renderOrderTab(state.activeOrderTab||'details')}refreshConnectionLabel()}
async function heartbeat(){if(!state.user)return;try{const r=await api('heartbeat');state.presence=r.presence||[];if(r.serverTime){const d=new Date(r.serverTime);if(!isNaN(d))state.serverOffsetMs=d.getTime()-Date.now()}renderPresence()}catch(_){}}
function startLiveSync(){stopLiveSync();const interval=Math.max(20000,Number(CFG.SYNC_INTERVAL_MS||25000));state.liveSyncTimer=setInterval(()=>{if(!state.user||state.visibleLoadActive||state.activeSyncPromise)return;if(document.hidden&&Date.now()-Number(state.lastSilentSync||0)<60000)return;loadData({silent:true})},interval);state.heartbeatTimer=setInterval(heartbeat,60000);state.connectionTimer=setInterval(refreshConnectionLabel,5000);heartbeat()}
function stopLiveSync(){if(state.liveSyncTimer)clearInterval(state.liveSyncTimer);if(state.heartbeatTimer)clearInterval(state.heartbeatTimer);if(state.connectionTimer)clearInterval(state.connectionTimer);state.liveSyncTimer=state.heartbeatTimer=state.connectionTimer=null}

async function setPage(p){closeTransientPanels();state.page=p;document.querySelectorAll('.sidebar [data-page]').forEach(b=>b.classList.toggle('active',b.dataset.page===p));const titles={dashboard:['Dashboard','Today’s operational overview and team workload'],advance:['Advance Picks','View and manage timed large-order collections'],small:['Small Orders','Manage small orders and walk-in collections'],board:['Picking Board','Prioritise and progress all picking work'],reports:['Reports','Customer service, picking and sales performance'],overdue:['Overdue Collections','Historic bookings that remain unresolved'],cancelled:['Cancelled Orders','Review recently cancelled bookings'],settings:['Settings','Manage your profile and application settings']};$('pageTitle').textContent=titles[p][0];$('pageSubtitle').textContent=titles[p][1];$('plannerControls').classList.toggle('hidden',['dashboard','reports','settings','overdue','cancelled'].includes(p));$('stats').classList.toggle('hidden',['settings','reports','overdue','cancelled'].includes(p));if(p==='dashboard'||p==='board'){state.day=operationalNow();state.viewMode='day'}else if(p==='advance'||p==='small'){state.weekStart=startOfWeek(operationalNow());state.viewMode='week';resetPlannerFilters()}if(p==='settings'){openSettings();return}if(p==='reports'){renderAll();return}if(p==='overdue'||p==='cancelled'){await renderOperationalList(p);return}await loadData({label:`Opening ${titles[p][0]}…`})}

function reportMarkup(r){const s=r.summary||{},days=r.byDay||{};return `<div class="stats" style="padding:16px;margin:0"><div class="stat"><span>Booked order value</span><strong>${money(s.bookedValueIncVAT||0)}</strong><small>All bookings in range</small></div><div class="stat"><span>Collected / invoiced</span><strong>${money(s.collectedValueIncVAT||0)}</strong><small>Collected orders</small></div><div class="stat"><span>Outstanding expected</span><strong>${money(s.outstandingValueIncVAT||0)}</strong><small>Not collected or cancelled</small></div><div class="stat"><span>Cancelled value</span><strong>${money(s.cancelledValueIncVAT||0)}</strong><small>Cancelled orders</small></div><div class="stat"><span>Average wait</span><strong>${s.averageWaitMinutes||0}m</strong><small>Waiting to collected</small></div><div class="stat"><span>Average pick</span><strong>${s.averagePickMinutes||0}m</strong><small>Picking to ready</small></div></div><table><thead><tr><th>Date</th><th>Bookings</th><th>Collected</th><th>Booked value</th><th>Collected value</th><th>Outstanding</th><th>Cancelled</th></tr></thead><tbody>${Object.keys(days).sort().map(d=>`<tr><td>${safe(d)}</td><td>${days[d].bookings}</td><td>${days[d].collected}</td><td>${money(days[d].bookedValueIncVAT||0)}</td><td>${money(days[d].collectedValueIncVAT||0)}</td><td>${money(days[d].outstandingValueIncVAT||0)}</td><td>${money(days[d].cancelledValueIncVAT||0)}</td></tr>`).join('')||'<tr><td colspan="7">No records in this period.</td></tr>'}</tbody></table>`}

$('cancelBookingBtn').onclick=cancelCurrentBooking;
$('panelReleased').onclick=()=>state.selected&&quickUpdate(state.selected.id,{accountStatus:'Released'},{source:'Quick action'});
$('panelPaid').onclick=async()=>{if(!state.selected)return;let type=state.selected.paymentType;if(!type)type=await paymentPrompt();if(!type)return;quickUpdate(state.selected.id,{paymentStatus:'Paid',paymentType:type},{source:'Quick action'})};
document.querySelectorAll('[data-order-tab]').forEach(x=>x.onclick=()=>renderOrderTab(x.dataset.orderTab));
document.querySelectorAll('[data-page]').forEach(x=>x.onclick=()=>setPage(x.dataset.page));
